...
@Component({
    ...
    imports: [NgClass, RouterModule, RippleModule, AuronIconComponent],
    template: `
        ...
        @if ((!item.routerLink || item.items) && item.visible !== false) {
            <a [attr.href]="item.url" (click)="itemClick($event)" [ngClass]="item.styleClass" [attr.target]="item.target" tabindex="0" pRipple>
                @if (item['auronIcon']) {
                    <auron-icon [name]="item['auronIcon']" [size]="18" class="layout-menuitem-icon"></auron-icon>
                } @else {
...
export class AppMenuitem {
    ...
    getTourKey(item: MenuItem): string | null {
        ...
        const map: Record<string, string> = {
            ...
            '/client/pos': 'menu-pos',
            '/client/products': 'menu-products',
            '/client/reports': 'menu-reports',
            '/client/clients': 'menu-clients',
            '/client/appointments': 'menu-appointments',
            '/client/payroll': 'menu-payroll',
            '/client/profile': 'menu-profile'
        };
