...
def employee_report(request):
    """Reporte básico de empleados - SOLO DATOS REALES"""
    ...
    # Empleados por especialidad REALES
    ...
    return Response({
        ...
        # top_performers REMOVIDO - era simulado
    })

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def sales_report(request):
    """Reporte básico de ventas - SOLO DATOS REALES"""
    ...
    # Ventas del mes REALES
    monthly_sales = Sale.objects.filter(
        user__tenant=tenant,
        date_time__gte=month_start
    ).aggregate(total=Sum('total'))['total'] or 0
    
    # Ventas por día (últimos 7 días) REALES
    sales_by_day = []
    for i in range(7):
    ...
    return Response({
        ...
        # top_services REMOVIDO - era hardcodeado
    })

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def dashboard_stats(request):
    """Estadísticas para dashboard - SOLO DATOS REALES"""
...
def services_performance(request):
    """Rendimiento de servicios - SOLO DATOS REALES"""
    ...
    # Top servicios por ventas REALES
